 ...
 * Мета: Створити функцію trackMousePosition, 
 * яка встановлює обробник події для відстеження руху миші по документу та виводить в консоль координати курсору миші (X та Y).
 *
 ...
 * 3. Вивід даних: При спрацьовуванні події mousemove, 
 * функція має виводити рядок у форматі `"Mouse X: [X], Mouse Y: [Y]"`, де `[X]` та `[Y]` - це відповідні координати курсору миші.
 *
 */
function trackMousePosition() {
const handlemMousemove = (event) => {
 ...
 * 
 * Мета: Створити функцію setupEventDelegation, 
 * яка дозволить встановити обробник подій на весь список, замість окремих елементів `<li>`. 
 * 
 * Функція повинна відстежувати кліки на елементах <li> у межах заданого списку 
 * і логувати текст "Item clicked: [Текст Елемента]", 
 ...
 * 3. Логування кліків: Коли елемент <li> клікнуто,
 *  
 * функція має вивести у консоль повідомлення у форматі "Item clicked: [Текст Елемента]", 
...
const handleUlClick = (event) => {
    if(event.target.tagName === "LI") {
        const textLi = event.target.textContent.trim();
        const numberLi = textLi.split(" ")[1];
        console.log(`Item clicked: ${numberLi}`);
    }
};
testlist.addEventListener("click", handleUlClick);
}
 ...
 * Мета: Розробити функцію, яка призначає обробник події кліку на кнопку з певним ID 
 * і виводить у консоль заздалегідь визначене повідомлення при кожному кліку на кнопку.
 *
 * Вимоги:
...
buttonEx1 = document.querySelector(`#${buttonId}`);
buttonEx1.addEventListener("click", callback);
}
